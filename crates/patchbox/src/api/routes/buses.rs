...
use crate::api::{GainBody, EnabledBody, MutedBody, PolarityBody, parse_bus_id, input_dsp_to_value, ws_broadcast};
use patchbox_core::config::{FilterConfig, EqConfig, GateConfig, CompressorConfig, InternalBusConfig};
...
pub(crate) struct CreateBusRequest { name: Option<String> }

#[derive(serde::Deserialize)]
pub(crate) struct UpdateBusRequest { name: Option<String>, muted: Option<bool> }

#[derive(serde::Deserialize)]
pub(crate) struct BusRoutingBody { routing: Vec<bool> }

#[derive(serde::Deserialize)]
pub(crate) struct BusMatrixBody { matrix: Vec<Vec<bool>> }

#[derive(serde::Deserialize)]
pub(crate) struct BusInputGainBody { rx: usize, gain_db: f32 }
...
// POST /api/v1/buses
pub(crate) async fn post_bus(State(s): State<AppState>, Json(body): Json<CreateBusRequest>) -> impl IntoResponse {
...
// GET /api/v1/buses/:id
pub(crate) async fn get_bus(State(s): State<AppState>, Path(id): Path<String>) -> impl IntoResponse {
...
// PUT /api/v1/buses/:id
pub(crate) async fn put_bus(State(s): State<AppState>, Path(id): Path<String>, Json(body): Json<UpdateBusRequest>) -> impl IntoResponse {
    let Some(i) = parse_bus_id(&id) else {
        return (StatusCode::BAD_REQUEST, "invalid bus id (expected bus_N)").into_response();
    };
    let mut cfg = s.config.write().await;
...
pub(crate) async fn delete_bus(State(s): State<AppState>, Path(id): Path<String>) -> impl IntoResponse {
    let Some(i) = parse_bus_id(&id) else {
        return (StatusCode::BAD_REQUEST, "invalid bus id (expected bus_N)").into_response();
    };
    let mut cfg = s.config.write().await;
...
pub(crate) async fn put_bus_gain(State(s): State<AppState>, Path(id): Path<String>, Json(body): Json<GainBody>) -> impl IntoResponse {
...
pub(crate) async fn put_bus_polarity(State(s): State<AppState>, Path(id): Path<String>, Json(body): Json<PolarityBody>) -> impl IntoResponse {
    let Some(i) = parse_bus_id(&id) else { return StatusCode::BAD_REQUEST.into_response(); };
    let mut cfg = s.config.write().await;
    let Some(bus) = cfg.internal_buses.get_mut(i) else { return StatusCode::NOT_FOUND.into_response(); };
...
pub(crate) async fn put_bus_hpf(State(s): State<AppState>, Path(id): Path<String>, Json(body): Json<FilterConfig>) -> impl IntoResponse {
    let Some(i) = parse_bus_id(&id) else { return StatusCode::BAD_REQUEST.into_response(); };
    let mut cfg = s.config.write().await;
    let Some(bus) = cfg.internal_buses.get_mut(i) else { return StatusCode::NOT_FOUND.into_response(); };
    bus.dsp.hpf = body;
...
pub(crate) async fn put_bus_lpf(State(s): State<AppState>, Path(id): Path<String>, Json(body): Json<FilterConfig>) -> impl IntoResponse {
    let Some(i) = parse_bus_id(&id) else { return StatusCode::BAD_REQUEST.into_response(); };
    let mut cfg = s.config.write().await;
    let Some(bus) = cfg.internal_buses.get_mut(i) else { return StatusCode::NOT_FOUND.into_response(); };
    bus.dsp.lpf = body;
...
pub(crate) async fn put_bus_eq(State(s): State<AppState>, Path(id): Path<String>, Json(body): Json<EqConfig>) -> impl IntoResponse {
    let Some(i) = parse_bus_id(&id) else { return StatusCode::BAD_REQUEST.into_response(); };
    let mut cfg = s.config.write().await;
    let Some(bus) = cfg.internal_buses.get_mut(i) else { return StatusCode::NOT_FOUND.into_response(); };
    bus.dsp.eq = body;
...
pub(crate) async fn put_bus_eq_enabled(State(s): State<AppState>, Path(id): Path<String>, Json(body): Json<EnabledBody>) -> impl IntoResponse {
    let Some(i) = parse_bus_id(&id) else { return StatusCode::BAD_REQUEST.into_response(); };
    let mut cfg = s.config.write().await;
    let Some(bus) = cfg.internal_buses.get_mut(i) else { return StatusCode::NOT_FOUND.into_response(); };
...
pub(crate) async fn put_bus_gate(State(s): State<AppState>, Path(id): Path<String>, Json(body): Json<GateConfig>) -> impl IntoResponse {
    let Some(i) = parse_bus_id(&id) else { return StatusCode::BAD_REQUEST.into_response(); };
    let mut cfg = s.config.write().await;
    let Some(bus) = cfg.internal_buses.get_mut(i) else { return StatusCode::NOT_FOUND.into_response(); };
    bus.dsp.gate = body;
...
pub(crate) async fn put_bus_compressor(State(s): State<AppState>, Path(id): Path<String>, Json(body): Json<CompressorConfig>) -> impl IntoResponse {
    let Some(i) = parse_bus_id(&id) else { return StatusCode::BAD_REQUEST.into_response(); };
    let mut cfg = s.config.write().await;
    let Some(bus) = cfg.internal_buses.get_mut(i) else { return StatusCode::NOT_FOUND.into_response(); };
...
pub(crate) async fn put_bus_mute(State(s): State<AppState>, Path(id): Path<String>, Json(body): Json<MutedBody>) -> impl IntoResponse {
    let Some(i) = parse_bus_id(&id) else { return StatusCode::BAD_REQUEST.into_response(); };
    let mut cfg = s.config.write().await;
    let Some(bus) = cfg.internal_buses.get_mut(i) else { return StatusCode::NOT_FOUND.into_response(); };
...
pub(crate) async fn put_bus_routing(State(s): State<AppState>, Path(id): Path<String>, Json(body): Json<BusRoutingBody>) -> impl IntoResponse {
    let Some(i) = parse_bus_id(&id) else { return StatusCode::BAD_REQUEST.into_response(); };
    let mut cfg = s.config.write().await;
...
pub(crate) async fn put_bus_input_gain(State(s): State<AppState>, Path(id): Path<String>, Json(body): Json<BusInputGainBody>) -> impl IntoResponse {
    let Some(i) = parse_bus_id(&id) else { return StatusCode::BAD_REQUEST.into_response(); };
    let mut cfg = s.config.write().await;
    if i >= cfg.internal_buses.len() { return StatusCode::NOT_FOUND.into_response(); }
...
pub(crate) async fn put_bus_matrix(State(s): State<AppState>, Json(body): Json<BusMatrixBody>) -> impl IntoResponse {
    let mut cfg = s.config.write().await;
...
// PUT /api/v1/bus-feed
pub(crate) async fn put_bus_feed(State(s): State<AppState>, Json(body): Json<BusFeedBody>) -> impl IntoResponse {
